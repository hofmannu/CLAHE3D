...
/// \brief a struct holding the settings for the histogram equilization
struct histeqsett
{
	std::size_t nBins = 255; //!< number of histogram bins used for equiliyation
	float noiseLevel = 0.1f; //!< lower cutoff intensity for histogram
	std::size_t sizeSubVols[3] = {11, 11, 11}; //!< size of the histogram subvolumes in elements
	std::size_t spacingSubVols[3] = {5, 5, 5}; //!< distance between the centers of the subvolumes
};
...
private:
	...
	// overwrite behaviour
	bool flagOverwrite = false; //!< should we put output over input matrix?
	float* dataOutput; // this will be used to store the output if overwrite is disabled
	bool isDataOutputAlloc = false;
