...
{
	const uint64_t iSub[3] = { // get index of current subvolume
		threadIdx.x + blockIdx.x * blockDim.x,
		threadIdx.y + blockIdx.y * blockDim.y,
		threadIdx.z + blockIdx.z * blockDim.z
	};
		...
		(iSub[2] < inArgs.nSubVols[2]))
	{
		...
		uint64_t startIdx[3];	uint64_t stopIdx[3];
		#pragma unroll
		for (uint8_t iDim = 0; iDim < 3; iDim++)
		{
			const uint64_t ctr = iSub[iDim] * inArgs.spacingSubVols[iDim] + inArgs.origin[iDim];
		...
		for (uint64_t iBin = 0; iBin < inArgs.nBins; iBin++) // reset bins
			localCdf[iBin] = 0;
