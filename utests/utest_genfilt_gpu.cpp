...
TEST_CASE("genfilt GPU preserves input and matches a hand-computed convolution", "[genfilt][gpu]")
{
	srand(time(NULL));
	...
	// validate the convolution at a single position
	float testVal = 0;
	...
	INFO("reference value (CPU) = " << testVal << ", GPU result = " << valueProc);
	REQUIRE(errorAmount <= 1e-5f);

	// the CPU path should still run afterwards
	REQUIRE_NOTHROW(myFilt.conv());
